...
static bool    WriteJsonPkt(int32 FileHandle, const PKTTBL_Pkt* Pkt, bool    FirstPktWritten);
...
*/
static bool    PktCallback (void* UserData, int TokenIdx);
static bool    FilterCallback (void* UserData, int TokenIdx);
...
void PKTTBL_SetTblToUnused(PKTTBL_Tbl* TblPtr)
{
   ...
   for (ApId=0; ApId < PKTTBL_MAX_APP_ID; ApId++) {
      ...
      TblPtr->Pkt[ApId].StreamId    = PKTTBL_UNUSED_MSG_ID;
      TblPtr->Pkt[ApId].Filter.Type = PKTUTIL_FILTER_ALWAYS;
...
*/
bool    PKTTBL_LoadCmd(TBLMGR_Tbl *Tbl, uint8 LoadType, const char* Filename)
{
      ...
      */
      if (PktTbl->AttrErrCnt == 0) {
         ...
         if (PktTbl->PktLoadCnt > 0) {
            ...
            if (LoadType == TBLMGR_LOAD_TBL_REPLACE) {
               ...
               if (PktTbl->JsonObj[PKTTBL_OBJ_PKT].Modified) {
                  ...
                  for (ApId=0; (ApId < PKTTBL_MAX_APP_ID) && (PktTbl->LastLoadStatus == TBLMGR_STATUS_VALID); ApId++) {
                          
                     if (PktTbl->Tbl.Pkt[ApId].StreamId != PKTTBL_UNUSED_MSG_ID) {

                        if (!(PktTbl->LoadTblEntryFunc)(ApId, (PKTTBL_Pkt*)PktTbl->JsonObj[PKTTBL_OBJ_PKT].UserData))
                  ...
                  CFE_EVS_SendEvent(PKTTBL_LOAD_EMPTY_ERR_EID, CFE_EVS_EventType_ERROR, "Load packet table update rejected.");
               ...
               CFE_EVS_SendEvent(PKTTBL_LOAD_TYPE_ERR_EID,CFE_EVS_EventType_ERROR,"Load packet table rejected. Invalid table command load type %d", LoadType);            
            ...
            CFE_EVS_SendEvent(PKTTBL_LOAD_UPDATE_ERR_EID, CFE_EVS_EventType_ERROR,
         ...
         if (PktTbl->PktLoadCnt != PktTbl->FilterLoadCnt) {
            ...
            CFE_EVS_SendEvent(PKTTBL_LOAD_UNDEF_FILTERS_EID, CFE_EVS_EventType_INFORMATION, 
      ...
      CFE_EVS_SendEvent(PKTTBL_LOAD_OPEN_ERR_EID,CFE_EVS_EventType_ERROR,"Load packet table open failure for file %s. File Status = %s JSMN Status = %s",
...
bool PKTTBL_DumpCmd(TBLMGR_Tbl *Tbl, uint8 DumpType, const char* Filename)
{
   ...
   if (OsStatus == OS_SUCCESS) {
      ...
      */
      ...
      for (ApId=0; ApId < PKTTBL_MAX_APP_ID; ApId++) {
         ...
         if (WriteJsonPkt(FileHandle, &(PktTblPtr->Pkt[ApId]), FirstPktWritten)) FirstPktWritten = true;
...
static bool    PktCallback (void* UserData, int TokenIdx)
{
   ...
   if (AttributeCnt == 4) {
      ...
      PktTbl->CurApId = Pkt.StreamId & PKTTBL_APP_ID_MASK;     
      PktTbl->Tbl.Pkt[PktTbl->CurApId] = Pkt;   
      ...
      ++PktTbl->AttrErrCnt;     
      CFE_EVS_SendEvent(PKTTBL_LOAD_PKT_ATTR_ERR_EID, CFE_EVS_EventType_ERROR, "Invalid number of packet attributes %d. Should be 4.",
                        AttributeCnt);
...
static bool    FilterCallback (void* UserData, int TokenIdx)
{
   ...
   CFE_EVS_SendEvent(KIT_TO_INIT_DEBUG_EID, KIT_TO_INIT_EVS_TYPE,
                     ...
                     PktTbl->CurApId, PktTbl->PktLoadCnt, PktTbl->AttrErrCnt, TokenIdx);
   ...
   if (AttributeCnt == 4) {
      ...
      PktTbl->Tbl.Pkt[PktTbl->CurApId].Filter = Filter;   
