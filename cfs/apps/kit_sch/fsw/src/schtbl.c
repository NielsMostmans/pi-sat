...
*/
static bool SlotCallback (void* UserData, int TokenIdx);
static bool ActivityCallback (void* UserData, int TokenIdx);
...
*/
bool SCHTBL_LoadCmd(TBLMGR_Tbl *Tbl, uint8 LoadType, const char* Filename)
{

   int entry;
   
   CFE_EVS_SendEvent(KIT_SCH_INIT_DEBUG_EID, KIT_SCH_INIT_EVS_TYPE, "SCHTBL_LoadCmd() Entry. sizeof(SchTbl->Tbl) = %ld\n",sizeof(SchTbl->Tbl));
      ...
      */
      if (SchTbl->AttrErrCnt == 0) {
         ...
         if (SchTbl->ObjLoadCnt > 0) {
            ...
            if (LoadType == TBLMGR_LOAD_TBL_REPLACE) {
               ...
               */
               if (SchTbl->ObjLoadCnt > 0 ) {
                  ...
                  CFE_EVS_SendEvent(SCHTBL_LOAD_EMPTY_ERR_EID,CFE_EVS_EventType_ERROR,"SCHTBL: Invalid table command Must have at least one slot(%d) adn activity(%d)",
               ...
               CFE_EVS_SendEvent(SCHTBL_LOAD_TYPE_ERR_EID,CFE_EVS_EventType_ERROR,"SCHTBL: Invalid table command load type %d",LoadType);            
            ...
            CFE_EVS_SendEvent(SCHTBL_LOAD_EMPTY_ERR_EID,CFE_EVS_EventType_ERROR,"SCHTBL: Invalid table command. No slots/activities objects defined.");
      ...
      CFE_EVS_SendEvent(SCHTBL_LOAD_OPEN_ERR_EID,CFE_EVS_EventType_ERROR,"SCHTBL: Table open failure for file %s. File Status = %s JSMN Status = %s",
...
bool SCHTBL_DumpCmd(TBLMGR_Tbl *Tbl, uint8 DumpType, const char* Filename)
{
   ...
   if (OsStatus == OS_SUCCESS) {
      ...
      */
      ...
      CFE_EVS_SendEvent(SCHTBL_CREATE_FILE_ERR_EID, CFE_EVS_EventType_ERROR,
   ...
   if (RetStatus) {
      ...
      CFE_EVS_SendEvent(SCHTBL_DUMP_INFO_EID, CFE_EVS_EventType_INFORMATION,
...
**
*/
bool SCHTBL_GetEntryPtr(uint16  EntryId, SCHTBL_Entry **EntryPtr)
{
                          ...
                          uint16 Offset, uint16 MsgTblIndex)
{
   ...
   if (CMDMGR_ValidBoolArg(Enabled)) {
      ...
      */
      ...
      CFE_EVS_SendEvent(SCHTBL_ENABLED_ERR_EID, CFE_EVS_EventType_ERROR,
                        "%s. Invalid Enabled value %d. Must be True(%d) or False(%d)",
                        EventStr, Enabled, true, false);    
         
   } /* End if invalid bool    config */
   ...
   */
   ...
   if (AttributeCnt == 1) {
      ...
      if (SlotIdx >= 0 && SlotIdx < SCHTBL_SLOTS) {
         ...
         CFE_EVS_SendEvent (SCHTBL_LOAD_ATTR_ERR_EID, CFE_EVS_EventType_ERROR, 
      ...
      SchTbl->AttrErrCnt++;     
      CFE_EVS_SendEvent(SCHTBL_LOAD_ATTR_ERR_EID, CFE_EVS_EventType_ERROR, "Scheduler Table error. Invalid number of slot attributes %d. Should be 1.",
                        AttributeCnt);
   ...
   if (SchTbl->CurSlotIdx != SCHTBL_UNDEF_SLOT) {
      ...
      */
      ...
      if (AttributeCnt == 5) {
         ...
         SchTbl->AttrErrCnt++;     
         CFE_EVS_SendEvent(SCHTBL_LOAD_PARSE_ERR_EID, CFE_EVS_EventType_ERROR, "Invalid number of activity attributes %d. Should be 5.",
                           AttributeCnt);
