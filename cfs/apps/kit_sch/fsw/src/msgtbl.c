...
*/
static bool MsgCallback (void* UserData, int TokenIdx);
...
*/
bool MSGTBL_LoadCmd(TBLMGR_Tbl *Tbl, uint8 LoadType, const char* Filename)
{
      ...
      */
      if (MsgTbl->AttrErrCnt == 0) {
         ...
         if (MsgTbl->ObjLoadCnt > 0) {
            ...
            if (LoadType == TBLMGR_LOAD_TBL_REPLACE) {
               ...
               */
               if (MsgTbl->ObjLoadCnt > 0 ) {
                  ...
                  CFE_EVS_SendEvent(MSGTBL_LOAD_EMPTY_ERR_EID,CFE_EVS_EventType_ERROR,"MSGTBL: Invalid table command. No messages defined.");
               ...
               CFE_EVS_SendEvent(MSGTBL_LOAD_TYPE_ERR_EID,CFE_EVS_EventType_ERROR,"MSGTBL: Invalid table command load type %d",LoadType);            
            ...
            CFE_EVS_SendEvent(MSGTBL_LOAD_EMPTY_ERR_EID,CFE_EVS_EventType_ERROR,"MSGTBL: Invalid table command. No message objects defined.");
      ...
      CFE_EVS_SendEvent(MSGTBL_LOAD_OPEN_ERR_EID,CFE_EVS_EventType_ERROR,"MSGTBL: Table open failure for file %s. File Status = %s JSMN Status = %s",
...
bool MSGTBL_DumpCmd(TBLMGR_Tbl *Tbl, uint8 DumpType, const char* Filename)
{
   ...
   if (OsStatus == OS_SUCCESS) {
      ...
      */
      ...
      CFE_EVS_SendEvent(MSGTBL_CREATE_FILE_ERR_EID, CFE_EVS_EventType_ERROR,
                        "Error creating dump file '%s', Status=0x%08X", Filename, OsStatus);
   
   } /* End if file create error */

   if (RetStatus) {
      
      CFE_EVS_SendEvent(MSGTBL_DUMP_INFO_EID, CFE_EVS_EventType_INFORMATION,
...
**
*/
bool MSGTBL_GetMsgPtr(uint16  EntryId, uint16 **MsgPtr)
{
   ...
   if (EntryId < MSGTBL_MAX_ENTRIES) {
      ...
      CFE_EVS_SendEvent(MSGTBL_DEBUG_EID, CFE_EVS_EventType_DEBUG,"MSGTBL_GetMsgPtr(): EntryId = %d, Buffer[0] = 0x%04x", EntryId, MsgTbl->Tbl.Entry[EntryId].Buffer[0]);
...
static bool MsgCallback (void* UserData, int TokenIdx)
{
   ...
   */
   ...
   if ( (DataWords  && AttributeCnt == 5) ||
      ...
      MsgTbl->AttrErrCnt++;     
      CFE_EVS_SendEvent(MSGTBL_LOAD_MSG_ATTR_ERR_EID, CFE_EVS_EventType_ERROR, "Invalid number of packet attributes %d. Should be 5.",
                        AttributeCnt);
